...
import { Search, X, Receipt } from "lucide-react"; // <-- Added Receipt icon
...
const Header = () => {
  ...
  // --- NEW: Handle Request Bill ---
  const handleRequestBill = () => {
    const storedTable = localStorage.getItem("stunner_table_number");

    if (!storedTable) {
      toast.error("⚠️ Table number not found. Please scan the QR code first.");
      return;
    }

    const confirmBill = window.confirm(
      `Request the bill for Table ${storedTable}?`,
    );

    if (confirmBill) {
      const deviceId = localStorage.getItem("stunner_device_id");
      ...
      // Feedback to user
      toast.success(
        `✅ Bill requested for Table ${storedTable}. A waiter will be with you shortly!`,
      );
    }
  };
  ...
  return (
    <>
      {/* HEADER */}
      <header className="bg-white p-3 md:p-4 shadow-md sticky top-0 z-30">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 md:gap-4 shrink-0">
            ...
            {/* --- NEW: Request Bill Button --- */}
            <button
              onClick={handleRequestBill}
              className="flex items-center gap-1.5 bg-orange-100 hover:bg-orange-200 text-orange-600 px-3 py-2 rounded-xl text-xs font-bold transition-colors shadow-sm"
              title="Request Bill"
            >
              {/* <Receipt size={18} /> */}
              <span className="hidden sm:inline">Req Bill</span>
            </button>
